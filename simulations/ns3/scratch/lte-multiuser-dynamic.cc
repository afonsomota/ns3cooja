...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue("ns3coojaConfig","NS3Cooja configuration file path",configFile);
  ...
  for (uint32_t u = 1; u < numberOfUeNodes; ++u) {
      ++ulPort;
  ...
  if(interPacketInterval > 0) clientApps.Start (Seconds (2));
